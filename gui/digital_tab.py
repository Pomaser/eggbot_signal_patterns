"""Digital signal tab — drop-in tk.Frame for use inside a ttk.Notebook."""
...
# Colour per signal type for the preview stroke
TYPE_COLORS = {
    "square":    "#c0392b",
    "sawtooth":  "#e67e22",
    "triangle":  "#27ae60",
...
class DigitalPanel(tk.Frame):
    def __init__(self, parent: tk.Widget, individual: DigitalIndividual, **kw):
        ...
        self._type_label = tk.Label(
            ctrl, text="", bg=BG, font=("sans-serif", 9, "bold"), width=10, anchor="w",
        )
    ...
    def _save_svg(self) -> None:
        ...
        existing = {p.stem for p in SVG_DIR.glob("digital_*.svg")}
        n = 1
        while f"digital_{n:04d}" in existing:
            n += 1
        path = SVG_DIR / f"digital_{n:04d}.svg"
        try:
    ...
    def draw(self) -> None:
        ...
        sig = self.individual.signal_type
        color = TYPE_COLORS.get(sig, "#1a1a1a")
        self._type_label.config(text=sig, fg=color)
...
class DigitalTab(tk.Frame):
    """Complete digital-signal tab content; embed in a ttk.Notebook."""

    def __init__(self, parent: tk.Widget, **kw):
        super().__init__(parent, bg=BG, **kw)
        self.generation = DigitalGeneration(NUM_INDIVIDUALS)
        self.panels: list[DigitalPanel] = []
    ...
    def _build_toolbar(self) -> None:
        ...
        # Legend
        for sig, color in TYPE_COLORS.items():
            tk.Label(bar, text=f"■ {sig}", fg=color, bg="#2c3e50",
    ...
    def _populate(self) -> None:
        ...
        for i, ind in enumerate(self.generation.individuals):
            ...
            panel = DigitalPanel(self.inner, ind)
